...
1. Fork the repository and add **exactly one new file**, `plugins/<pluginId>/versions/<version>.json`.
   ...
   `version` (semver), `pluginApi` (a non-negative integer), `fliks` (a range, with a mandatory
...
2. Open the PR. Automated checks refuse it long before anything would need to download your
   archive: the file's shape against the catalog's schema, that your declared `pluginApi` has a row
   in `COMPATIBILITY.md`, that your `fliks` range actually has an upper bound, and that rebuilding
   the whole catalog from the `plugins/` tree is deterministic.
3. A maintainer reviews and merges. Merging **is** the release: a workflow regenerates
   `catalog.json` from every `versions/*.json` file, signs it with the catalog's own key, and
   republishes it. There's no separate release step.

Your own archive, hosted at the `zipUrl` you gave, is signed with **your own** key (or unsigned).
Only the catalog's *document* is signed with the catalog's key here, that's what lets an admin trust
the list itself; it says nothing about the individual plugin archives it points at. An install from
...
2. Optionally sign it: Ed25519, over the document's exact raw bytes (no reformatting after
   signing), the signature published as base64 text at `<your-catalog-url>.sig`.
3. In Fliks, **Settings > Plugins > Manage sources > Add source**, give the catalog's URL and,
   if you signed it, the base64 of your Ed25519 **public** key (32 raw bytes) as the source's
   pinned key.
...
> updates, which only ever take `official`-signed versions. This is the same "revocation authority
> is signing authority" model that governs a `denyList` (below): a key only vouches for, and can
> only revoke, what it itself signed.
...
scopes, it never changes an existing one's shape or meaning. Core keeps accepting an older
`pluginApi` value for a whole release cycle after a newer one ships, that window is exactly when
you republish; only once a later release actually drops the old value from
`SUPPORTED_PLUGIN_API_VERSIONS` does a plugin still declaring it stop installing, with
`incompatible-api`.
...
release key revokes anything it signed; a source pinned to its own key revokes only what *that* key
signed; an unsigned or manually-imported package is revocable by nobody, because nobody vouched for
it in the first place. This falls directly out of the trust model above with no separate mechanism:
a catalog's key is already the thing an admin chose to trust when they added the source.
